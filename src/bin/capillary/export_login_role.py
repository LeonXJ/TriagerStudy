...
for login in actors:
	...
	if actor.is_maint == True:
		conclusion += lib.sep + '0' + lib.fsep + 'maint'
	else:
		# assume she was reporter at the very beginning
		conclusion += lib.sep + '0' + lib.fsep + 'self'
		if actor.nonself_since < actor.dev_since:
			conclusion += lib.tsep + str(actor.nonself_since) + lib.fsep + 'triager'
		if actor.dev_since < INFINIT:
			conclusion += lib.tsep + str(actor.dev_since) + lib.fsep + 'dev'
	print conclusion
