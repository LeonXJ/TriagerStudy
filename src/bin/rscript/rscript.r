...
calculate_metrics <- function(act_assi){
	act_assi$AllErrRat <- 1.0 - act_assi$nAllCorAssi/act_assi$nAllAssi
	act_assi$AllErrRat[is.na(act_assi$AllErrRat)] <- 0.5
	act_assi$PErr <- 1.0 - act_assi$nPrdCorAssi/act_assi$nPrdAssi
	act_assi$PErr[is.na(act_assi$PErr)] <- 0.5
	act_assi$TrErr <- 1.0 - act_assi$nLogCorAssi/act_assi$nLogAssi
	act_assi$TrErr[is.na(act_assi$TrErr)] <- 0.5
	act_assi$TrPErr <- 1.0 - act_assi$nLogPrdCorAssi/act_assi$nLogPrdAssi
	act_assi$TrPErr[is.na(act_assi$TrPErr)] <- 0.5
	
	act_assi$ExpP <- act_assi$ExpPrdAssi
	act_assi$Exp <- act_assi$ExpP + act_assi$ExpNotPrdAssi
...
show_assigning_model <- function(tdata, begin_year=2001, end_year=2009){
	...
	model_assi$correlation <- round(cor(fac_assi[,c("correct","Exp", "ExpP","SNSize", "AvgSNDep","PErr","TrErr","TrPErr","MaxSNExp")], method="spearman"),2)
	
	glm_assi <- glm(correct~PErr+TrPErr+log(MaxSNExp+1)+log(SNSize+1)+log(AvgSNDep+1)+P, data=fac_assi, family=binomial)
	anova_assi <- anova(glm_assi)
	model_assi$explain_deviance <- sum(anova_assi[-1,2])/anova_assi[1,4]
	
	summary_assi <- summary(glm_assi)$coefficients[1:7,c(1,4)]
	ExpDev <- anova_assi[,2]
...
show_predict <- function(tdata, learn_begin_year=2008, learn_end_year=2009, test_begin_year=2009, test_end_year=2010){
	...
	glm_learn <- glm(I(correct)~I(PErr)+I(TrPErr)+I(log(MaxSNExp+1))+I(log(SNSize+1))+I(log(AvgSNDep+1))+I(P), data=learn_set, family=binomial)
	...
	tt<-table(test_set$prd<=quantile(test_set$prd,.05),!test_set$correct)
	pnr05 <- c(tt[2,2]/(tt[2,1]+tt[2,2]),tt[2,2]/(tt[1,2]+tt[2,2]))
	pnrr <- tt[2,2]/(sum(tt)*tt[2,2]/sum(tt[,2]))

	tt<-table(test_set$prd<=quantile(test_set$prd,.1),!test_set$correct)
	pnr10 <- c(tt[2,2]/(tt[2,1]+tt[2,2]),tt[2,2]/(tt[1,2]+tt[2,2]))
	
	prdpnr <- data.frame(t(cbind(pnr05, pnr10, pnrr)))
	names(prdpnr) <- c("Precision", "Recall")
	row.names(prdpnr) <- c("Threshold .05", "Threshold .10", "Random Pick")
	prediction$Precision_and_Recall <- prdpnr
...
run <- function(community, data_dir = "../../data/"){
	...
	if(community == "mozilla" | community == "mozillaNew"){
